...
namespace FetcoCall.Controllers
{
    public class WorkorderController : Controller
    {
        ...
        {
            using (FetcoEntities s = new FetcoEntities())
            {
        ...
        public JsonResult SaveWorkorder(CustomerServiceModel CustomerDetails)
        {
            ...
            if(string.IsNullOrEmpty(CustomerDetails.FetcoCustomerNumber) && string.IsNullOrEmpty(CustomerDetails.PaymentTransactionId))
            {
            ...
            if(!string.IsNullOrEmpty(CustomerDetails.FetcoCustomerNumber) && CustomerDetails.PaymentTerm.ToLower() == "credit card" && string.IsNullOrEmpty(CustomerDetails.PaymentTransactionId))
            {
                message = @"|Payment Process is Incomplete!";
                returnValue = -1;
                isValid = false;
            ...
            if (isValid == true)
            {
                using (FetcoEntities fetcoEntity = new FetcoEntities())
                {
                    ...
                    NonFBCustomer nonFBCust = fetcoEntity.NonFBCustomers.Where(f => f.NonFBCustomerName.ToLower() == "fetprotect").FirstOrDefault();
                    if (nonFBCust != null)
                    {
                        customerdata.ParentNumber = nonFBCust.NonFBCustomerId;
                    }

                    returnValue = WorkOrderSave(customerdata, fetcoEntity, out workOrder, out message);

                    if (CustomerDetails.AccountNumber == 0)
                    {
                        CustomerModel cust = new CustomerModel();

                        cust.CreateUnknownCustomer(customerdata, fetcoEntity);
                        workOrder.CustomerID = Convert.ToInt32(customerdata.CustomerId);
                        fetcoEntity.SaveChanges();
                    }
        ...
        public int WorkOrderSave(CustomerModel customerdata, FetcoEntities fetcoEntity, out WorkOrder workOrder, out string message)
        {
            int returnValue = 0;
            message = string.Empty;
            workOrder = null;
            ...
            WorkorderType WOTyp = fetcoEntity.WorkorderTypes.Where(w => w.CallTypeID == customerdata.ServiceType).FirstOrDefault();
            ...
            Contact serviceCustomer = fetcoEntity.Contacts.Where(x => x.ContactID == CustomerId).FirstOrDefault();

            workOrder = workorderManagement.FillCustomerData(new WorkOrder(), true, fetcoEntity, customerdata, serviceCustomer);


            IndexCounter counter = Utility.GetIndexCounter("WorkorderID", 1);
            counter.IndexValue++;
            fetcoEntity.Entry(counter).State = System.Data.Entity.EntityState.Modified;            
            ...
            AllFBStatu priority = fetcoEntity.AllFBStatus.Where(p => p.FBStatus.ToLower() == "next day service").FirstOrDefault();
            int priorityStatusId = 52;// priority == null ? 0 : priority.FBStatusID;
            ...
            //TimeZoneInfo newTimeZoneInfo = null;
            //Utility.GetCustomerTimeZone(workorderManagement.Customer.ZipCode, MarsServiceEntitites);
            DateTime CurrentTime = Utility.GetCurrentTime(customerdata.ZipCode, fetcoEntity);
            workOrder.WorkorderEntryDate = CurrentTime;
            ...
            fetcoEntity.NotesHistories.Add(customerNotes);
...

            {
                ...
                NotesHistory notesHistory = new NotesHistory()
                {
                    AutomaticNotes = 1,
                    EntryDate = CurrentTime,
                    Notes = "Work Order created from Fetco Web, WO#: " + workOrder.WorkorderID + " !",
                    Userid = 99999,
                    UserName = "Fetco WEB",
                    WorkorderID = workOrder.WorkorderID
            };
                ...
                //nts.Text = "Event created from Fetco Web";
                ...
                assetCounter.IndexValue++;
                fetcoEntity.Entry(assetCounter).State = System.Data.Entity.EntityState.Modified;
                ...
                notesHistory = new NotesHistory()
                {
                    AutomaticNotes = 1,
                    EntryDate = workOrder.WorkorderEntryDate,
                    Notes = workOrder.WorkorderCalltypeDesc + " Work Order # " + workOrder.WorkorderID + " in Fetco!",
                    Userid = 99999,
            ...
            fetcoEntity.WorkOrders.Add(workOrder);
            SaveRemovalDetails(workorderManagement, workOrder, fetcoEntity);
            ...
            //    if (workOrderDetail.CustomerSignatureDetails.Length == 890)
            //    {
            //        workOrderDetail.CustomerSignatureDetails = fetcoEntity.WorkorderDetails.Where(w => w.WorkorderID == workorderManagement.WorkOrder.WorkorderID).
            //            Select(s => s.CustomerSignatureDetails).FirstOrDefault();
            //        if (workOrderDetail.CustomerSignatureDetails != null)
            //        {
            //            if (workOrderDetail.CustomerSignatureDetails.Length == 890)
            ...
            fetcoEntity.WorkorderDetails.Add(workOrderDetail);
            workOrder.CurrentUserName = "Fetco WEB";

            int effectedRecords = fetcoEntity.SaveChanges();
            returnValue = effectedRecords > 0 ? 1 : 0;

            return returnValue;
        }

        private void SaveRemovalDetails(WorkorderManagementModel workorderManagement, WorkOrder workOrder, FetcoEntities fetcoEntity)
        {
            if (workorderManagement.RemovalCount > 0)
            {
                AllFBStatu status = fetcoEntity.AllFBStatus.Where(a => a.FBStatusID == workorderManagement.RemovalReason).FirstOrDefault();
                RemovalSurvey survey = fetcoEntity.RemovalSurveys.Where(r => r.WorkorderID == workOrder.WorkorderID).FirstOrDefault();
                if (survey != null)
                {
                ...
                else
                {
                    ...
                    fetcoEntity.RemovalSurveys.Add(newSurvey);

                    TimeZoneInfo newTimeZoneInfo = null;
                    Utility.GetCustomerTimeZone(workorderManagement.Customer.ZipCode, fetcoEntity);
                    DateTime currentTime = Utility.GetCurrentTime(workorderManagement.Customer.ZipCode, fetcoEntity);
                ...
                if (workorderManagement.RemovalCount > 1 && workorderManagement.RowId.HasValue && workorderManagement.RowId.Value < workorderManagement.WorkOrderEquipmentsRequested.Count())
                {
                    ...
                    IEnumerable<WorkorderEquipmentRequested> workOrderEquipments = fetcoEntity.WorkorderEquipmentRequesteds.Where(we => we.WorkorderID == workOrder.WorkorderID);
        ...
        public JsonResult GetServiceQuote(string ZipCode, string State, string ServiceType)
        {
            ...
            using (FetcoEntities fetcoEntity = new FetcoEntities())
            {
                int ClosestTechId = GetClosestTech(ZipCode, fetcoEntity);

                TECH_HIERARCHY techHView = fetcoEntity.TECH_HIERARCHY.Where(t => t.DealerId == ClosestTechId).FirstOrDefault();
                if (techHView != null)
                {
                    dynamic travelDetails = Utility.GetTravelDetailsBetweenZipCodes(techHView.PostalCode, ZipCode);

                    NonFBCustomer nonFBCust = fetcoEntity.NonFBCustomers.Where(f => f.NonFBCustomerName.ToLower() == "fetco").FirstOrDefault();
                    string ParentId = "0";
                    if (nonFBCust != null)
                    {
                    ...
                    PricingDetail priceDtls = Utility.GetPricingDetails(ParentId, ClosestTechId, State, fetcoEntity); //For UnknownCustomer CustomerId is 0
        ...
        private int GetClosestTech(string postCode, FetcoEntities fetcoEntity)
        {
            int TechID = -1;
            DateTime currentTime = Utility.GetCurrentTime(postCode, fetcoEntity);
            TechID = getAvailableTechId(postCode, currentTime, fetcoEntity);
        ...
        private int getAvailableTechId(string PostalCode, DateTime currentTime, FetcoEntities fetcoEntity)
        {
            ...
            foreach (DataRow dr in rsReferralList.Rows)
            {
                ...
                bool IsUnavailable = IsTechUnAvailable(techId, currentTime, fetcoEntity, out replaceTechId);
        ...
        public bool IsTechUnAvailable(int techId, DateTime StartTime, FetcoEntities fetcoEntity, out int replaceTech)
        {
                ...
                foreach (TechSchedule holiday in holidays)
                {
                    ...
                    if ((UnavailableStartDate <= StartTime) && (UnavailableEndDate > StartTime))
                    {
                        if (holiday.ReplaceTech != null && holiday.ReplaceTech != 0)
                        {
                            ...
                            IsTechUnAvailable(replaceTech, StartTime, fetcoEntity, out replaceTech);
                        }
                        else
                        { return true; }
                    }
        ...
        public bool SendMail(int WorkOrderID, FetcoEntities fetcoEntity)
        {
            WorkOrder wo = fetcoEntity.WorkOrders.Where(w => w.WorkorderID == WorkOrderID).FirstOrDefault();
            DateTime currentTime = Utility.GetCurrentTime(wo.CustomerZipCode, fetcoEntity);

            string fromAddress = "reviveservice@mktalt.com"; 

            int cid = Convert.ToInt32(wo.CustomerID);
            Contact customer = fetcoEntity.Contacts.Where(c => c.ContactID == cid).FirstOrDefault();
            ...
            mailTo += ConfigurationManager.AppSettings["supportEmailId"].ToString();
                mailToName += "Fetco Protect - " + ConfigurationManager.AppSettings["supportEmailId"].ToString() + ";    ";

                NotesMsg += "Fetco Event Create Mail sent to " + mailToName;
                ...
                using (var smtp = new SmtpClient())
                {
                    ...
                    foreach (string address in addresses)
                    {
                        if (!string.IsNullOrWhiteSpace(address))
                        {
                            ...
                            IEnumerable<NotesHistory> histories = fetcoEntity.NotesHistories.Where(w => w.WorkorderID == WorkOrderID).OrderByDescending(n => n.EntryDate);
                            ...
                            string logoPath = Path.Combine(HttpRuntime.AppDomainAppPath, "images/Fetco.png");
