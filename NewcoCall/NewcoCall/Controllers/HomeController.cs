...
namespace NewcoCall.Controllers
{
    ...
    {
        NewcoEntity newcoEntity = new NewcoEntity();
        ...
        public ActionResult Index()
        {
            ...
            List<FBCallReason> fbcallreasonlist = newcoEntity.FBCallReasons.Where(c => DbFunctions.TruncateTime(c.DateActive) < DbFunctions.TruncateTime(currentTime)
            ...
            csm.StateList = newcoEntity.States.OrderBy(s => s.StateName).ToList();
            ...
            List<WorkorderType> WOTypeList = newcoEntity.WorkorderTypes.Where(w => w.Active == 1 && callTypeIds.Contains(w.CallTypeID)).ToList();
        ...
        public JsonResult SaveNonServiceEvent([ModelBinder(typeof(CustomerServiceModelBinder))]CustomerServiceModel NonService)
        {
            ...
            else
            {
                if (returnValue >= 0)
                {
                    SendMail(workOrder.WorkOrderID, newcoEntity);
        ...
        public int NonServiceWorkOrderSave(CustomerServiceModel NonService, CustomerModel customerdata, out NonServiceworkorder workOrder, out string message)
        {
            ...
            //if (NonService.Operation == WorkOrderManagementSubmitType.SAVE)
            {
                try
                {
                    ...
                    DateTime currentTime = Utility.GetCurrentTime(NonService.PostalCode, newcoEntity);
                    ...
                    if (NonService.WorkorderId == 0)
                    {
                        ...
                        if (!nsw.CustomerID.HasValue || nsw.CustomerID.Value <= 0)
                        {
                            ...
                            custmodel.CreateUnknownCustomer(customerdata, newcoEntity);
                        ...
                        newcoEntity.NonServiceworkorders.Add(nsw);
                        workOrder = nsw;
                    ...
                    returnValue = newcoEntity.SaveChanges();
                }
                catch (Exception ex)
                {
        ...
        public bool SendMail(int WorkOrderID, NewcoEntity ReviveEntitites)
        {
        ...
        [HttpGet]
        public JsonResult GetCustomerDetails(int CustomerNumber)
        {
            JsonResult jsonResult = new JsonResult();

            List<string> newcoParentIdsList = newcoEntity.NonFBCustomers.Where(c => c.NonFBCustomerName.Contains("newco")).Select(s=>s.NonFBCustomerId).ToList();

            Contact contact = newcoEntity.Contacts.Where(c => c.ContactID == CustomerNumber && newcoParentIdsList.Contains(c.PricingParentID)).FirstOrDefault();
            ...
            {
                jsonResult.Data = new { success = false, serverError = ErrorCode.ERROR, message = "Please Enter a Valid Newco Customer !" };
            }
