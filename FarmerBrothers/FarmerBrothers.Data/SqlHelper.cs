...
namespace FarmerBrothers.Data
{
    public class SqlHelper
    {
        ...
        public DataTable GetAssetExceptionReportData(params string[] arguemnts)
        {
            DataSet ds = new DataSet("AssetExceptionReprot");
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand sqlComm = new SqlCommand("USP_AssetException_Report", conn))
                {
                    sqlComm.CommandTimeout = 0;
                    sqlComm.Parameters.AddWithValue("@StartDate", arguemnts[0]);
                    sqlComm.Parameters.AddWithValue("@EndDate", arguemnts[1]);
