...
namespace FarmerBrothers.Models
{
    public class WorkorderManagementModelBinder : IModelBinder
    {
                                ...
                                ModelBindingContext bindingContext)
        {
            ...
            //if (!string.IsNullOrWhiteSpace(request.Unvalidated.Form.Get("Customer.IsNonFBUser")))
            //{
            //    model.Customer.IsNonFBUser = Convert.ToBoolean(request.Unvalidated.Form.Get("Customer.IsNonFBUser"));
            //}

            foreach (var property in model.Customer.GetType().GetProperties())
            {
                if(property.Name.ToLower() == "isnonfbcustomer")
                {
    ...
    public class NonServiceEventModelBinder : IModelBinder
    {
        public object BindModel(ControllerContext controllerContext,
                                ModelBindingContext bindingContext)
        {
            ...
            NonServiceEventModel model = new NonServiceEventModel();
            ...
            if (request.Unvalidated.Form.Get("IsAutoDispatched") != null)
                IsAutoDispatchedCheckValue = (request.Unvalidated.Form.Get("IsAutoDispatched").Split(','))[0];
            if (IsAutoDispatchedCheckValue.Contains("true"))
                model.Notes.IsAutoDispatched = true;
            else
                model.Notes.IsAutoDispatched = false;
            ...
            if (!string.IsNullOrWhiteSpace(request.Unvalidated.Form.Get("Zipcode")))
            {
                model.Customer.ZipCode = Convert.ToString(request.Unvalidated.Form.Get("Zipcode"));
            }
