...
{
    public class CallCloserModelBinder : IModelBinder
    {
                                ...
                                ModelBindingContext bindingContext)
        {
            ...
            IList<WorkOrderManagementEquipmentModel> woEqp = null;
            JavaScriptSerializer json_serializer = new JavaScriptSerializer();

            model.Customer = new CustomerModel();
            model.WorkOrder = new Data.WorkOrder();
            ...
            {
                woEqp = json_serializer.Deserialize<IList<WorkOrderManagementEquipmentModel>>(request.Unvalidated.Form.Get("WorkOrderClosureHidden"));


                IList<IList<WorkOrderPartModel>> woPart;
                ...
                string[] weights = null;
                if (!string.IsNullOrWhiteSpace(request.Unvalidated.Form.Get("WeightsHidden")))
                {
                    string value = json_serializer.Deserialize<string>(request.Unvalidated.Form.Get("WeightsHidden"));
                    weights = value.Split('|');
                }

                string[] ratios = null;
                if (!string.IsNullOrWhiteSpace(request.Unvalidated.Form.Get("RatiosHidden")))
                {
                    string value = json_serializer.Deserialize<string>(request.Unvalidated.Form.Get("RatiosHidden"));
                    ratios = value.Split('|');
                }
                ...
                for (int i = 0; i < woEqp.Count; i++)
                {
                    ...
                    {
                        woEqp[i].Weight = weights[i];
                    }
                    if (ratios != null && ratios.Length > i)
                    {
                        woEqp[i].Ratio = ratios[i];
                    }
                ...
                WorkOrderClosureModel woClosure = new WorkOrderClosureModel();
                {
                    ...
                    //woClosure.CustomerSignature = Encoding.ASCII.GetBytes(request.Unvalidated.Form.Get("Closure.CustomerSignature"));
                    woClosure.CustomerSignatureDetails = request.Unvalidated.Form.Get("CustomerSignatureDetailsHidden");
                    woClosure.TechnicianSignatureDetails = request.Unvalidated.Form.Get("TechnicianSignatureDetailsHidden");
                    woClosure.CustomerSignedBy = request.Unvalidated.Form.Get("Closure.CustomerSignedBy");
            ...
            foreach (var property in model.WorkOrder.GetType().GetProperties())
            {
                ...
                if (property.Name == "WorkorderID")
                {
                    string[] workorders = request.Unvalidated.Form.Get("WorkOrder." + property.Name).Split(',');
            ...
            model.WorkOrder.WorkorderCallstatus = "Accepted";
            ...
            if (!string.IsNullOrWhiteSpace(request.Unvalidated.Form.Get("ReasonCode")))
            {
                model.ReasonCode = Convert.ToInt32(request.Unvalidated.Form.Get("ReasonCode"));
            }

            //WorkorderController wc = new WorkorderController();
            //int customerId = Convert.ToInt32(model.Customer.CustomerId);
            //int workorderId = model.WorkOrder.WorkorderID;
            //WorkOrderClosureModel wrcloser = new WorkOrderClosureModel();
            //wrcloser = model.Closure;
            ////IList<WorkOrderManagementEquipmentModel> wrEqup = new List<WorkOrderManagementEquipmentModel>();
            ////wrEqup = woEqp;
